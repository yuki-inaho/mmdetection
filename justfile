# mmdetection cu12 (torch 2.1.0 + cu121) task runner.
#
#   just sync        # provision .venv (cu12 deps) + put this mmdet source on the path (.pth)
#   just smoke       # import mmdet (version guards) + bbox_overlaps on GPU
#   just env-doctor  # print python/torch/mmcv/mmengine/mmdet/gpu state
...
# Provision the cu12 env (deps) and make this mmdet source importable (.pth).
sync:
    uv sync
...
env-doctor:
    ...
    @if [ -x "{{ PY }}" ]; then \
        "{{ PY }}" --version; \
        for pkg in torch mmcv mmengine mmdet numpy; do \
...
# Run the mmdet GPU smoke (import guards + bbox_overlaps on cuda).
smoke OUT="/tmp/smoke_mmdet_cu12.json": sync
    "{{ PY }}" tools/cu12/smoke_mmdet.py --output-json "{{ OUT }}"
