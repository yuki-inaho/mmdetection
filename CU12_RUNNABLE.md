...
**Python 3.10 / torch 2.1.0+cu121 / mmcv 2.2.0 / mmengine 0.10.7 / mmdet 3.3.0**.
...
wheel; `mmcv<2.3.0` / `mmengine<1.0.0` (mmdet's `__init__.py` guards) are satisfied
by mmcv 2.2.0 + mmengine 0.10.7. mmdet is pure Python (no native ext), so `just sync`
